...
void retirar_tarefa_por_posicao2(Lista *lista, int categoria)
{
    Node* aux = lista->inicio;
    Node* temp;

    while (aux != NULL) {
        if (categoria == 1 && aux->tarefa.categoria <= 15) {
            temp = aux;
            aux = aux->proximo;
            ...
            delete(temp);
            lista->tamanho--;
        } else if (categoria == 2 && aux->tarefa.categoria > 15) {
            temp = aux;
            aux = aux->proximo;
...
void buscar_tarefas_por_categoria(Lista *lista, int categoria)
{
    ...
    if (categoria == 1) {
        ...
        while (aux != NULL) {
            if (aux->tarefa.categoria <= 15) {
                cout << aux->tarefa.tarefa << endl;
            }
            aux = aux->proximo;
        }
    } else if (categoria == 2) {
